...
import {
  ...
  hoppOverUtdanningBestattOgGodkjentSporsmalene,
  spmSomIkkeSkalBesvaresConfig,
} from "./skjema-config";
...
export function svarSuffiksTilTekstId(svar: Svar) {
  return svar.toString().toLowerCase().split("_").join("-");
}
...
export function finnNesteSporsmalPlassering(props: SkjemaProps): number {
  ...
  const foregaendeSporsmalIder = getSporsmalIder(props).filter(
    (sporsmalId, indeks) => indeks <= gjeldendeSporsmalPlassering
  );

  const sporsmalIderSomIkkeSkalBesvares = getAlleSporsmalSomIkkeSkalBesvares(
    foregaendeSporsmalIder,
    props.svarState,
    props.config
  );
...
export function nullStillSporsmalSomIkkeSkalBesvares(
  sporsmalId: any,
  svar: any,
  endreSvar: any,
  resetSvar: (spmId: string) => void
) {
