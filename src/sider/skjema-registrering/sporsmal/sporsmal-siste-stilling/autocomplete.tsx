    ...
    return (
      ...
      >
        {this.props.resultatListe.map((e, i) => {
          return (
            <li aria-selected="false" data-stilling-index={i} key={i} role="option" tabIndex={-1}>
              {e.labelKey}
            </li>
          );
        })}
...
class AutoComplete extends React.Component<Props, AutoCompleteState> {
  ...
  componentDidUpdate() {
    ...
    if (ariaLive) {
      ariaLive.textContent = numberResults === 0 ? "" : numberResults + " resultater er tilgjengelige.";
    }
  }
  ...
  setFormRef(node: HTMLFormElement | null) {
    this.formRef = node;
  }
  ...
  arrowing(kc: number) {
    ...
    if (resultat) {
      if (kc === keyboard.down) {
        nextMenuItem = indexElement !== undefined ? resultat.children[indexElement + 1] : resultat.children[0];
      }
  ...
  render() {
    ...
    return (
      <form className="autocomplete-form" ref={this.setFormRef} action="">
        ...
        <span id="initInstr">
          Når resultatene er tilgjengelige bruk piltastene til å navigere og enter for å velge. På mobile enheter, trykk
          eller sveip.
        </span>
