...
import { nyArbeidsgiverSporsmaleneConfig } from "./skjema-sykefravaer-sporsmalene";
...
class SkjemaSykefravaerNyArbeidsgiver extends React.Component<Props> {
  render() {
    ...
    const sporsmalene = nyArbeidsgiverSporsmaleneConfig(fellesProps, regType).map((spmElement) => spmElement.element);

    const inngangsLoepSvar: FremtidigSituasjonSvar = hentSvar(
      this.props.svarState,
      SporsmalId.fremtidigSituasjon
    ) as FremtidigSituasjonSvar;
...
export default connect(mapStateToProps, mapDispatchToProps)(injectIntl<Props>(SkjemaSykefravaerNyArbeidsgiver));
