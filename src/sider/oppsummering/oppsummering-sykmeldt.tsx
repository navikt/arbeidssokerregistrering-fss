...
class OppsummeringSykmeldt extends React.Component<Props> {
  ...
  handleNesteBtnClicked() {
    ...
    const svarTilBackend = mapAvgitteSvarForBackend(
      state.svar,
    ...
    this.props.onRegistrerBruker(svarTilBackend, RegistreringType.SYKMELDT_REGISTRERING).then(() => {
      const erRegistrert = this.props.state.registrerBruker.status === STATUS.OK;
